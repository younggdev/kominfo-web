...
if (isset($_FILES['foto']) && strlen($_FILES['foto']['name']) > 1) {
    ...
    define('F_NAME', preg_replace('/\.(.+?)$/i', '', basename($_FILES['foto']['name'])));
    ...
    if (in_array($type, $imgset['type'])) {
        ...
        if (isset($width) && isset($height)) {
            ...
            if ($_FILES['foto']['size'] > $imgset['maxsize'] * 1000) {
                $re .= '\\n Maximum file size must be: ' . $imgset['maxsize'] . ' KB.';
            }
        }
    } else {
        $re .= 'The file: ' . $_FILES['foto']['name'] . ' has not the allowed extension type.';
    }
...
$query = mysqli_query($koneksi, "INSERT INTO berita VALUES ('', '$judul', '$isi', '$nama_file', '$tanggal')");
